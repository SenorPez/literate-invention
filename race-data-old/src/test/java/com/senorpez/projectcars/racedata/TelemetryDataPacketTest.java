...
import com.senorpez.projectcars.racedata.TelemetryDataPacket.State;
...
public class TelemetryDataPacketTest {
    ...
    public static class StateTests {
        ...
        @Parameters(name = "Game: {0}, Race: {1}, Session: {2}, Result: {3}")
        public static Collection<Object[]> data() {
            final List<Object[]> output = new ArrayList<>();
            EnumSet.allOf(GameState.class)
                    .forEach(gameState -> EnumSet.allOf(RaceState.class)
                            .forEach(raceState -> EnumSet.allOf(SessionState.class)
                                    .forEach(sessionState -> {
                                        final State expectedState;
                                        if (raceState == RaceState.RACE_NOT_STARTED
                                                && gameState == GameState.GAME_INGAME_PAUSED) {
                                            expectedState = PRE_RACE_PAUSED;
                                        } else if (raceState == RaceState.RACE_NOT_STARTED
                                                && gameState == GameState.GAME_INGAME_PLAYING) {
                                            expectedState = PRE_RACE;
                                        } else if (raceState == RaceState.RACE_RACING) {
                                            expectedState = RACING;
                                        } else if (raceState == RaceState.RACE_FINISHED) {
                                            expectedState = FINISHED;
                                        } else if (gameState == GameState.GAME_MAX
                                                && raceState == RaceState.RACE_INVALID
                                                && sessionState == SessionState.SESSION_INVALID) {
                                            expectedState = LOADING;
                                        } else {
                                            expectedState = UNDEFINED;
                                        }
                                        output.add(new Object[]{gameState, raceState, sessionState, expectedState});
                                    })));
            return output;
        }
        ...
        public void GetState() throws Exception {
            final TelemetryDataPacketBuilder builder = new TelemetryDataPacketBuilder()
                    ...
                    .setExpectedSessionState(sessionState);
            packet = new TelemetryDataPacket(builder.build());
            assertThat(packet.getState(), is(resultState));
        }
    }

    @RunWith(Parameterized.class)
    public static class StateLoadingTests {
        private final State state = LOADING;
        ...
        @Parameters(name = "Previous State: {0}")
        public static Collection<Object[]> data() {
            final List<Object[]> output = new ArrayList<>();
            EnumSet.allOf(State.class).forEach(previousState -> {
                if (previousState == RACING) output.add(new Object[]{previousState, true});
                else output.add(new Object[]{previousState, false});
            });
            return output;
        }

        @Test
        public void GetRaceStarted() throws Exception {
            assertThat(state.raceStarted(), is(false));
        }

        @Test
        public void GetRaceFinished() throws Exception {
            assertThat(state.raceFinished(previousState), is(expectedValue));
        }

        @Test
        public void GetRaceFinishedTelemetryExhausted() throws Exception {
            assertThat(state.raceFinishedTelemetryExhausted(), is(false));
        }
    }

    @RunWith(Parameterized.class)
    public static class StatePreRacePausedTests {
        private final State state = PRE_RACE_PAUSED;
        ...
        public static Collection<Object[]> data() {
            final List<Object[]> output = new ArrayList<>();
            EnumSet.allOf(State.class).forEach(previousState -> {
                if (previousState == FINISHED) output.add(new Object[]{previousState, true});
                else output.add(new Object[]{previousState, false});
            });
            return output;
        }
    ...
    @RunWith(Parameterized.class)
    public static class StatePreRaceTests {
        private final State state = PRE_RACE;
        ...
        @Test
        public void GetRaceStarted() throws Exception {
            assertThat(state.raceStarted(), is(true));
        }
    ...
    @RunWith(Parameterized.class)
    public static class StateRacingTests {
        private final State state = RACING;
    ...
    @RunWith(Parameterized.class)
    public static class StateFinishedTests {
        private final State state = FINISHED;
        ...
        public static Collection<Object[]> data() {
            final List<Object[]> output = new ArrayList<>();
            EnumSet.allOf(State.class).forEach(previousState -> output.add(new Object[]{previousState, false}));
            return output;
        }
        ...
        @Test
        public void GetRaceFinishedTelemetryExhausted() throws Exception {
            assertThat(state.raceFinishedTelemetryExhausted(), is(true));
        }
    }

    @RunWith(Parameterized.class)
    public static class StateUndefinedTests {
        private final State state = UNDEFINED;
