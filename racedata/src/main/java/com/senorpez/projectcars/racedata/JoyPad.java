...
public enum JoyPad {
    ...
    JoyPad(final Integer stateValue) {
    ...
    JoyPad(final Integer stateValue, final String PCButtonName, final String XBoxButtonName, final String PS4ButtonName) {
        this.stateValue = stateValue;
    ...
    static {
        lookup.putAll(EnumSet.allOf(JoyPad.class)
                .stream()
                .collect(Collectors.toMap(joypad -> joypad.stateValue, Function.identity())));
    }

    static JoyPad valueOf(final int stateValue) {
        return lookup.get(stateValue);
    }

    Boolean isSet(final Integer joyPad) {
