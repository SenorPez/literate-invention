...
@SpringBootTest
public class CarClass2ControllerTest {
    private MockMvc mockMvc;
    private static final MediaType INVALID_MEDIA_TYPE = new MediaType("application", "invalid", UTF_8);
    private static final ClassLoader CLASS_LOADER = CarClass2ControllerTest.class.getClassLoader();
    ...
    public void getAllClasses_ValidAcceptHeader() throws Exception {
        ...
        mockMvc.perform(get("/classes").accept(PROJECT_CARS_2))
                ...
                .andExpect(jsonPath("$._embedded.pcars:class", hasItem(
                        allOf(
                                ...
                                hasEntry(equalTo("_links"),
                                        hasEntry(equalTo("self"),
                                                hasEntry("href", String.format("http://localhost:8080/classes/%d", SECOND_CLASS.getId()))))))))
                .andExpect(jsonPath("$._links.index", hasEntry("href", "http://localhost:8080/")))
                .andExpect(jsonPath("$._links.self", hasEntry("href", "http://localhost:8080/classes")))
                .andExpect(jsonPath("$._links.curies", everyItem(
                        allOf(
                                hasEntry("href", (Object) "http://localhost:8080/docs/{rel}"),
                                hasEntry("name", (Object) "pcars"),
                                hasEntry("templated", (Object) true)))))
                .andDo(document("classes2",
                        ...
                        responseFields(
                                fieldWithPath("_embedded.pcars:class").description("Class resources"),
                                fieldWithPath("_embedded.pcars:class[].id").description("ID number"),
                                fieldWithPath("_embedded.pcars:class[].name").description("Name"),
                                subsectionWithPath("_links").ignored(),
                                subsectionWithPath("_embedded.pcars:class[]._links").ignored()),
                        commonLinks));
    ...
    public void getAllClasses_InvalidMethod() throws Exception {
        ...
        mockMvc.perform(put("/classes").accept(PROJECT_CARS_2))
    ...
    public void getSingleClass_ValidId_ValidAcceptHeader() throws Exception {
        ...
        mockMvc.perform(get(String.format("/classes/%d", FIRST_CLASS.getId())).accept(PROJECT_CARS_2))
                .andExpect(status().isOk())
                .andExpect(content().contentType(PROJECT_CARS_2))
    ...
    public void getSingleClass_ValidId_InvalidMethod() throws Exception {
        ...
        mockMvc.perform(put(String.format("/classes/%d", FIRST_CLASS.getId())).accept(PROJECT_CARS_2))
                .andExpect(status().isMethodNotAllowed())
                .andExpect(content().contentType(APPLICATION_JSON_UTF8))
                .andExpect(content().string(matchesJsonSchema(ERROR_SCHEMA)))
                .andExpect(jsonPath("$.code", is(METHOD_NOT_ALLOWED.value())))
    ...
    public void getSingleClass_InvalidId_ValidAcceptHeader() throws Exception {
        ...
        mockMvc.perform(get("/classes/8675309").accept(PROJECT_CARS_2))
    ...
    public void getSingleClass_InvalidId_InvalidMethod() throws Exception {
        ...
        mockMvc.perform(put("/classes/8675309").accept(PROJECT_CARS_2))
                .andExpect(status().isMethodNotAllowed())
                .andExpect(content().contentType(APPLICATION_JSON_UTF8))
                .andExpect(content().string(matchesJsonSchema(ERROR_SCHEMA)))
                .andExpect(jsonPath("$.code", is(METHOD_NOT_ALLOWED.value())))
