...
class CarClassResource extends Resource<CarClassModel> {
    CarClassResource(final CarClassModel content, final Link... links) {
        super(content, links);
        this.add(linkTo(methodOn(CarClassController.class).carClasses1()).withRel("classes"));
    }
    ...
    static Resources<CarClassResource> makeResources(final Collection<CarClassResource> resources) {
        ...
        resources.forEach(carClassResource -> carClassResource.add(linkTo(methodOn(CarClassController.class).carClasses1(carClassResource.getContent().getId())).withSelfRel()));
        final Resources<CarClassResource> carClassResources = new Resources<>(resources);
        carClassResources.add(linkTo(methodOn(CarClassController.class).carClasses1()).withSelfRel());
