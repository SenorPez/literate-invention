...
class LiveryResource extends Resource<LiveryModel> {
    LiveryResource(final LiveryModel content, final int carId, final Link... links) {
        super(content, links);
        this.add(linkTo(methodOn(LiveryController.class).liveries1(carId)).withRel("liveries"));
    }
    ...
    static Resources<LiveryResource> makeResources(final Collection<LiveryResource> resources, final int carId) {
        ...
        resources.forEach(liveryResource -> liveryResource.add(linkTo(methodOn(LiveryController.class).liveries1(carId, liveryResource.getContent().getId())).withSelfRel()));
        final Resources<LiveryResource> liveryResources = new Resources<>(resources);
        liveryResources.add(linkTo(methodOn(LiveryController.class).liveries1(carId)).withSelfRel());
